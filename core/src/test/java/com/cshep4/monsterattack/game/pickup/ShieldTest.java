...
public class ShieldTest {
    ...
    @Test
    public void create() {
        Shield shield = Shield.create(X_POS, Y_POS, SIZE, SIZE);

        assertThat(shield, instanceOf(Shield.class));
        assertThat(shield.getX(), is(X_POS));
        assertThat(shield.getY(), is(Y_POS));
        assertThat(shield.getWidth(), is(SIZE));
        assertThat(shield.getHeight(), is(SIZE));
    }

    @Test
    public void reward_givesPlayerTenMoreSecondsOfShieldTime() {
        Player player = Player.create(X_POS, Y_POS);
        Shield shield = Shield.create(X_POS, Y_POS, SIZE, SIZE);

        int expectedShieldTime = player.getRemainingShieldTime() + SHIELD_TIME;

        shield.reward(player);

        assertThat(player.getRemainingShieldTime(), is(expectedShieldTime));
    }
}